...
import { FileUploadContentType, UploadFilesAction, UploadFilesState } from 'src/app/shared/store/files-upload';
...
export class GaleryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this._store.select(RoomState.selectStateRoom(this.data.room._id)).subscribe(async (value)=>{
      ...
      console.log("Data Structured ",data)
  ...
  ngOnDestroy(): void {
    console.log("ShouldResetFiles ",this.shouldResetFiles)
