    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--youpez-${e(key)}`, value], [`--youpez-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`youpez.customProps.background.${colorScheme}`);
            const foreground = theme(`youpez.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--youpez-${e(key)}`, value], [`--youpez-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--youpez-${e(key)}`, value], [`--youpez-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--youpez-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--youpez-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                youpez  : {
                    customProps: {
